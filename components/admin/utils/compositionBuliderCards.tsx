...
import { ClientWithImages, ProjectWithRelations, ProjectWithRelationsSlide, ServiceWithImage, TeamMemberWithImage, TestimonialWithImage } from "@/types/schema"
...
export const ServiceCard = ({ data }: { data: any }) => {
   ...
   return  <motion.div
    ...
    >
        {DataToRender.image && (
            <div className="mb-5 h-48 overflow-hidden rounded-xl bg-muted">
                <BlurredImage
...
export const ClientCard = ({ data }: { data: ClientWithImages }) => (
    <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
    ...
    >
        {data.logo && (
            <div className="h-32 bg-muted/50 flex items-center justify-center p-6 border-b border-border">
               <BlurredImage

                    imageUrl={data.logo.url || ""}
                    height={data.logo.height || 400}
                    width={data.logo.width || 800}
